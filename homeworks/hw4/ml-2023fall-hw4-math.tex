...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \Repeat
        ...
        \State $\boldsymbol{Y}^{t}_{i,j} = \boldsymbol{Y}^{t}_{i,j} / (\boldsymbol{Y}^{t}_{i,1} + \boldsymbol{Y}^{t}_{i,2}) \quad i=1,...,C, j = 1, 2$ \Comment{Normalize the probability distribution}
